...
[ ! -z $4 ] && INITIAL_IP=$4 || { echo "No initial network address specified. Assuming 10.11.0.0/16"; INITIAL_IP="10.11.0.0"; }
...
[[ $WORKPORT_BEGIN == $WORKPORT_END ]] && [[ $CONTAINERS_AMOUNT -gt 1 ]] && echo "Will create only single container with port=$WORKPORT_BEGIN"
...
for ((CONTAINER_COUNT=1;CONTAINER_COUNT<=CONTAINERS_AMOUNT;CONTAINER_COUNT++)); do
	...
	[[ $CURRENT_PORT -le $WORKPORT_END ]] || { echo "Container $CONTAINER_COUNT is out of range of available ports. Aborting execution"; exit 1; }
	OCTET2=$(( $(echo "$INITIAL_IP" | cut -d '.' -f2) + $CONTAINER_COUNT ))
	CURRENT_IP=$(echo "$INITIAL_IP" | sed "s/[0-9]\+\.\{1\}0$/$OCTET2\.0/" )
